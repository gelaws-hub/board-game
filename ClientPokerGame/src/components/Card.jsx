...
const Card = ({ suit, rank, isBackFacing = false, onClick }) => {
  ...
  return (
    <div
      className={`w-12 h-[4.5rem] md:w-14 md:h-20 p-[2px] md:p-1 rounded-md bg-white border border-gray-300 flex flex-col shadow-md cursor-pointer hover:shadow-lg transition-shadow`}
      onClick={() => onClick && onClick({ suit, rank })}
    >
      <div className={`text-xs sm:text-sm font-bold ${getSuitColor(suit)} flex justify-between`}>
        <span>{getRankDisplay(rank)}</span>
        <span>{getSuitSymbol(suit)}</span>
      </div>
      <div
        className={`flex-grow flex items-center justify-center ${getSuitColor(suit)} text-lg sm:text-xl md:text-2xl`}
      >
        {getSuitSymbol(suit)}
      </div>
      <div className={`text-xs sm:text-sm font-bold ${getSuitColor(suit)} flex justify-between rotate-180`}>
        <span>{getRankDisplay(rank)}</span>
        <span>{getSuitSymbol(suit)}</span>
      </div>
    </div>
